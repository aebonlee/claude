...
const glossaryTerms = [
  ...
  { term: '컨텍스트 윈도우 (Context Window)', termEn: 'Context Window', def: '모델이 한 번에 처리할 수 있는 최대 토큰 수. Opus 4.8·Sonnet 4.6·Fable 5는 최대 1M(100만) 토큰, Haiku 4.5는 200K 토큰을 지원합니다.', defEn: 'The maximum number of tokens a model can process at once. Opus 4.8, Sonnet 4.6, and Fable 5 support up to 1M (1 million) tokens; Haiku 4.5 supports 200K.' },
  ...
  { term: '적응형 사고 (Adaptive Thinking)', termEn: 'Adaptive Thinking', def: 'Claude가 언제·얼마나 깊이 사고할지 스스로 결정하는 최신 방식. 과거의 budget_tokens 방식을 대체합니다.', defEn: 'The current approach where Claude decides on its own when and how deeply to think, replacing the older budget_tokens method.' },
  { term: 'Effort 파라미터', termEn: 'Effort Parameter', def: '사고 깊이와 토큰 사용량을 조절하는 파라미터(low/medium/high/xhigh/max). 기본값은 high이며 코딩·에이전트 작업은 xhigh를 권장합니다.', defEn: 'A parameter that tunes thinking depth and token usage (low/medium/high/xhigh/max). Default is high; xhigh is recommended for coding and agentic work.' },
  { term: '프롬프트 캐싱 (Prompt Caching)', termEn: 'Prompt Caching', def: '반복되는 시스템 프롬프트나 긴 컨텍스트를 캐싱해 비용과 지연을 줄이는 기능. 캐시 읽기는 기본 입력가의 약 10%입니다.', defEn: 'Caching repeated system prompts or long context to cut cost and latency. Cache reads cost about 10% of the base input price.' },
  { term: '구조화 출력 (Structured Outputs)', termEn: 'Structured Outputs', def: 'JSON 스키마로 모델 응답 형식을 강제하는 기능. output_config.format으로 지정하며 prefill을 대체합니다.', defEn: 'Constrains the model response to a JSON schema via output_config.format, replacing assistant prefills.' },
  { term: '스킬 (Skills)', termEn: 'Skills', def: '작업에 필요할 때만 로드되는 전문 지침 묶음(SKILL.md). Claude를 특정 작업 전문가로 만듭니다.', defEn: 'Bundles of specialized instructions (SKILL.md) loaded only when a task needs them, turning Claude into a task specialist.' },
  { term: 'Managed Agents', termEn: 'Managed Agents', def: 'Anthropic이 에이전트 루프와 도구 실행 컨테이너를 호스팅하는 서버 관리형 에이전트. 에이전트를 1회 생성 후 세션을 반복 실행합니다.', defEn: 'Server-managed agents where Anthropic hosts the agent loop and a tool-execution container. Create the agent once, then run sessions repeatedly.' },
